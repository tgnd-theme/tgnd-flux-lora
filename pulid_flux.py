...
Based on guozinan/PuLID (PuLID-FLUX v0.9.1 weights for FLUX.1-dev, dim=3072).
...
PuLID weights have ~5 double CA + ~7 single CA modules mapped across blocks.
"""
...
class PerceiverAttentionCA(nn.Module):
    def __init__(self, dim=3072, dim_head=64, heads=16, kv_dim=None):
        super().__init__()
        if kv_dim is None:
            kv_dim = dim
...
class IDFormer(nn.Module):
    def __init__(self, dim=3072, dim_head=64, heads=16, num_tokens=4, num_layers=4):
        super().__init__()
...
class PuLIDFlux(nn.Module):
    def __init__(self, dim=3072, dim_head=64, heads=16, num_double_ca=5, num_single_ca=7):
        super().__init__()
        self.dim = dim
        self.dim_head = dim_head
        self.heads = heads
    ...
    def from_pretrained(cls, path, device="cpu"):
        """Load PuLID-FLUX weights from safetensors file."""
        from safetensors.torch import load_file

        state_dict = load_file(path, device=str(device))

        # Remap key names (some checkpoints use different prefixes)
        remapped = {}
...
def detect_flux_variant(transformer):
    """Detect Flux model variant and hidden dim from transformer."""
    ...
    # Best approach: read inner_dim directly from transformer config
    if hasattr(transformer, 'inner_dim'):
        return "auto", transformer.inner_dim, n_double, n_single
    ...
    if hasattr(transformer, 'config'):
        ...
        if heads and head_dim:
            return "auto", heads * head_dim, n_double, n_single

    # Fallback heuristics
    if n_double == 19 and n_single == 38:
        return "flux1_dev", 3072, n_double, n_single
    elif n_double <= 6 and n_single <= 22:
...
def patch_flux(transformer, pulid_module, id_tokens, strength=0.8):
    ...
    Args:
        transformer: FluxTransformer2DModel from diffusers pipeline
        pulid_module: PuLIDFlux instance with loaded weights
    ...
    # Handle dim mismatch (weights trained for different model variant)
    if pulid_dim != flux_dim:
        print(f"[PuLID] Dim mismatch: PuLID={pulid_dim}, Flux={flux_dim}. Projecting id_tokens.", flush=True)
    ...
    print(f"[PuLID] Patching {variant}: {n_double} double + {n_single} single blocks, dim={flux_dim}, strength={strength}", flush=True)
...
def load_pulid_model(weights_path, device="cuda"):
    ...
    model = PuLIDFlux.from_pretrained(weights_path, device=device)
